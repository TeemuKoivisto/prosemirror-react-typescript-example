...
function dedupe<T>(list: T[] = [], iteratee: (p: T) => T[keyof T] | T = (p) => p): T[] {
  /**
              .,
    .      _,'f----.._
    |\ ,-'"/  |     ,'
...
export const searchQuickInsertItems =
  (quickInsertState: QuickInsertPluginState, options?: QuickInsertPluginOptions) =>
  (query?: string, category?: string): QuickInsertItem[] => {
    const defaultItems =
      options && options.disableDefaultItems ? [] : quickInsertState.lazyDefaultItems()
