...
import { QuickInsertItem, QuickInsertProvider, ProviderFactory } from '../../provider-factory'
...
export const quickInsertPlugin = (options?: QuickInsertPluginOptions): EditorPlugin => ({
  name: 'quickInsert',
  ...
  pluginsOptions: {
    typeAhead: {
      ...
      getItems: (query, state, _tr, dispatch) => {
        const quickInsertState: QuickInsertPluginState = pluginKey.getState(state)
...
export const processItems = (items: Array<QuickInsertHandler>) => {
  if (!itemsCache['en']) {
    itemsCache['en'] = items.reduce((acc: Array<QuickInsertItem>, item: QuickInsertHandler) => {
