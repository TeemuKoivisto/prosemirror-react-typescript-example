...
export function insertTypeAheadQuery(trigger: string, replaceLastChar = false): Command {
  return (state, dispatch) => {
    ...
    dispatch(
      safeInsert(
        state.schema.text(trigger, [state.schema.marks.typeAheadQuery.create({ trigger })])
      )(state.tr)
    )
