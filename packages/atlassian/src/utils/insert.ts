...
import { Transaction, EditorState, NodeSelection, Selection } from 'prosemirror-state'
...
export const insertSelectedItem =
  (maybeNode?: PMNode | Object | string | Fragment, opts: { selectInlineNode?: boolean } = {}) =>
  (state: EditorState, tr: Transaction, start: number): Transaction => {
    ...
    if (node instanceof PMNode && node.isText) {
      ...
      const fragment =
        node instanceof Fragment ? node : Fragment.fromArray([node, state.schema.text(' ')])
