...
export const replaceNodeAtPos = (position: number, content: PMNode | Fragment) => (tr: Transaction) => {
  // @ts-ignore
...
export const replaceParentNodeOfType = (nodeType: NodeType | NodeType[], content: PMNode | Fragment) => (tr: Transaction) => {
...
export const setTextSelection = (position: number, dir = 1) => (tr: Transaction) => {
...
const isSelectableNode = (node: PMNode | Fragment) => node instanceof PMNode && node.type && node.type.spec.selectable;
const shouldSelectNode = (node: PMNode | Fragment) => node instanceof PMNode && isSelectableNode(node) && node.type.isLeaf;
...
// );
// ```
export const safeInsert = (content: PMNode | Fragment, position?: number, tryToReplace?: boolean) => (tr: Transaction) => {
